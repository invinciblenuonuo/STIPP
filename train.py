...
writer = SummaryWriter('./runs/trans_1')
...
def valid_epoch(data_loader, predictor, planner, use_planning):
    ...
    for batch in data_loader:
        ...
        ego = batch[0].to(args.device,non_blocking=True)
        neighbors = batch[1].to(args.device,non_blocking=True)
        map_lanes = batch[2].to(args.device,non_blocking=True)
        map_crosswalks = batch[3].to(args.device,non_blocking=True)
        ref_line_info = batch[4].to(args.device,non_blocking=True)
        ground_truth = batch[5].to(args.device,non_blocking=True)
        current_state = torch.cat([ego.unsqueeze(1), neighbors[..., :-1]], dim=1)[:, :, -1]
        weights = torch.ne(ground_truth[:, 1:, :, :3], 0)
...
def model_training():
    ...
    valid_loader = DataLoader(valid_set, batch_size=batch_size, shuffle=False, pin_memory=True,num_workers=args.num_workers)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_workers", type=int, default=36, help="number of workers used for dataloader")
